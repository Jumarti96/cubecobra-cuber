...
- Confirm it passes the `card_pool_rules` check and that it is **usable** within the colour constraint. For a card whose printed `color_identity` is off-colour, do not defend it on identity — state the mode that makes it legal (its `usable_as`, e.g. "cycled for 2 life — colourless", "cast base, kicker declined") and confirm that mode's cost is within `core_colors ∪ splash_colors`.
...
4. **Identity fit** — does each card contribute to the pipeline the deck was built around? Suggest cuts that do not. **Colour legality is by usable mode, not printed identity:** before raising an off-colour inclusion as a finding, check its `usable_as` — a card played through a colourless/in-colour mode (a cycler, a kicker-decline) is legal even though its `color_identity` prints off-colour, and `effective_cost.best_mode(card, core_colors, splash_colors)` returns non-`None` for it. Raise a BLOCKING colour finding only when `best_mode` is `None` (no usable mode fits the colours) or when a card tagged for an alternate mode is nonetheless being counted on for its off-colour cast (value stripped — the mode it is legal by is not the mode the plan needs).
...
6. **Proportional validation** — check `build_output.slot_allocation` against accepted deckbuilding ranges for `build_output.macro_archetype`. Flag deviations lacking adequate rationale. Then audit the Step-0 record `build_output.skeleton_selection`: it must be present; the chosen build's band deviations must carry the thesis grounds the judge credited (`judge_grounds`); every `weak_keystones` entry must have been resolved in the final list — the flagged card replaced, or still present with a thesis-grounded justification (an unresolved one is a finding); and the sketches must be builds of **one** archetype — a `rejected_sketches` entry whose macro-archetype differs from `archetype_family` means Step 0 spanned archetypes instead of interpreting one (a finding). You audit the recorded selection; you do not re-run it.
