...
6. **Proportional validation** — check `build_output.slot_allocation` against accepted deckbuilding ranges for `build_output.macro_archetype`. Flag deviations lacking adequate rationale. Then audit the Step-0 record `build_output.skeleton_selection`: it must be present; the chosen shape's band deviations must carry the thesis grounds the judge credited (`judge_grounds`); and every `weak_keystones` entry must have been resolved in the final list — the flagged card replaced, or still present with a thesis-grounded justification (an unresolved one is a finding). You audit the recorded selection; you do not re-run it.
